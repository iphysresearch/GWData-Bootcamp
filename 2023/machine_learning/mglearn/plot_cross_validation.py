...
def plot_label_kfold():
    try:
        ...
        from sklearn.model_selection import GroupKFold as LabelKFold
    ...
    try:
        ...
        for i, (train, test) in enumerate(cv.split(range(12), groups=labels)):
            mask[i, train] = 1
            mask[i, test] = 2
    ...
    for i in range(n_folds):
        ...
        try:
            ...
            boxes = axes.barh(y=range(n_splits), width=[1 - 0.1] * n_splits, left=i * n_samples_per_fold, height=.6, color=colors, hatch="//")
        for j in np.where(mask[:, i] == 0)[0]:
            boxes[j].set_hatch("")
    
    try:
        axes.barh(bottom=[n_splits] * n_folds, width=[1 - 0.1] * n_folds, left=np.arange(n_folds) * n_samples_per_fold, height=.6, color="w")
    except:
        axes.barh(y=[n_splits] * n_folds, width=[1 - 0.1] * n_folds, left=np.arange(n_folds) * n_samples_per_fold, height=.6, color="w")
    ...
    axes.set_yticklabels(["Split %d" % x for x in range(1, n_splits + 1)]# + ["labels"]
                        );
